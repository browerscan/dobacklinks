...
async function checkSample() {
  ...
  result.forEach((p, i) => {
    ...
    console.log(`   Ahrefs Organic Traffic: ${p.ahrefsOrganicTraffic?.toLocaleString() || "N/A"}`);
    console.log(`   Referral Domains: ${p.referralDomains?.toLocaleString() || "N/A"}`);
    console.log(`   SEMrush AS: ${p.semrushAS || "N/A"}`);
    console.log(`   SEMrush Traffic: ${p.semrushTotalTraffic?.toLocaleString() || "N/A"}`);
