...
console.log(`   Cloudflare Account: ${CLOUDFLARE_ACCOUNT_ID ? "✅ Configured" : "❌ Missing"}`);
console.log(`   API Token: ${CLOUDFLARE_API_TOKEN ? "✅ Configured" : "❌ Missing"}`);
...
if (!CLOUDFLARE_ACCOUNT_ID || !CLOUDFLARE_API_TOKEN || !DATABASE_URL) {
  ...
  console.error("   Please set CLOUDFLARE_ACCOUNT_ID, CLOUDFLARE_API_TOKEN, and DATABASE_URL");
  process.exit(1);
}
...
console.log(`   Estimated time: ${LIMIT * 10} seconds (~${Math.ceil((LIMIT * 10) / 60)} minutes)`);
console.log("");
console.log("📝 To run this script:");
console.log("   1. First ensure database migration is complete: pnpm db:push");
console.log("   2. Then run: node scripts/batch-capture-screenshots.js --limit 10");
console.log("");
